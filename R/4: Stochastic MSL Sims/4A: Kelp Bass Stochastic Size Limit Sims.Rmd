---
title: "Kelp Bass Stochastic Size Limit Sims"
output: pdf_document
date: "2022-12-06"
editor_options: 
...
```{r}
#2004-2012
LHKB_PRE <- (KB_LH_PRE_LBSPR.LinfMK[["LH_inputs"]])
LHKB_PRE$FM <- KB_LH_PRE_LBSPR.LinfMK[["F_M"]]



#2013-2021
LHKB_POST <- (KB_LH_POST_LBSPR.LinfMK[["LH_inputs"]])
LHKB_POST$FM <- KB_LH_POST_LBSPR.LinfMK[["F_M"]]
...
```{r}
LHKB_PRE$SPR_12_Inch_MS_KE <- NA



  for(i in 1:nrow(LHKB_PRE)){
    if(is.na(LHKB_PRE$FM[i])){
    LHKB_PRE$SPR_12_Inch_MS_KE[i] <- NA }
else{
...
MySim_KelpBass_12_KE <- LBSPRsim(KBPars)
...
####
quantile(LHKB_PRE$SPR_12_Inch_MS_KE, probs = c(0.025,0.25,0.5,0.75,0.975), na.rm = T)
...
```{r}
LHKB_PRE$SPR_14_Inch_MS_KE <- NA



  for(i in 1:nrow(LHKB_PRE)){
    if(is.na(LHKB_PRE$FM[i])){
    LHKB_PRE$SPR_14_Inch_MS_KE[i] <- NA }
else{
...
MySim_KelpBass_14_KE <- LBSPRsim(KBPars)


LHKB_PRE$SPR_14_Inch_MS_KE[i] <- MySim_KelpBass_14_KE@SPR
}}


####
quantile(LHKB_PRE$SPR_14_Inch_MS_KE, probs = c(0.025,0.25,0.5,0.75,0.975), na.rm = T)
...
```{r}
KelpBass_sizelimits <- data.frame(cbind(KB_LH_PRE_LBSPR.LinfMK$SPR, LHKB_PRE$SPR_12_Inch_MS_KE, KB_LH_POST_LBSPR.LinfMK$SPR, LHKB_PRE$SPR_14_Inch_MS_KE))
colnames(KelpBass_sizelimits) <- c("2004-2012 CRFS", "12 Inch Knife-Edge MSL Simulation", "2013-2021 CRFS", "14 Inch Knife-Edge MSL Simulation")
KelpBass_sizelimits_gg<-melt(KelpBass_sizelimits,variable.name="Data",value.name="SPR")
colnames(KelpBass_sizelimits_gg)<-c("Scenario","SPR")



#VIOLIN
ggplot(KelpBass_sizelimits_gg,aes(Scenario,SPR, group=Scenario)) +
  ...
  ggtitle("Kelp Bass Size Limit Simulations")+
  theme(plot.title=element_text(hjust=0.5))

#ggsave("KelpBass_Stochastic_MSLsim.png", width = 30, height = 20, units = "cm", dpi = 500)
```

#16 INCH MSL knife edge --use 2013-2021 FM values 
```{r}
LHKB_POST$SPR_16_Inch_MS_KE <- NA



  for(i in 1:nrow(LHKB_POST)){
    if(is.na(LHKB_POST$FM[i])){
    LHKB_POST$SPR_16_Inch_MS_KE[i] <- NA }
else{
    #for each value of FM, put into sim
KBPars <- new("LB_pars")
KBPars@Species <- "Kelp Bass"
...
MySim_KelpBass_16_KE <- LBSPRsim(KBPars)


LHKB_POST$SPR_16_Inch_MS_KE[i] <- MySim_KelpBass_16_KE@SPR
}}


####
quantile(LHKB_POST$SPR_16_Inch_MS_KE, probs = c(0.025,0.25,0.5,0.75,0.975), na.rm = T)
...
colnames(KelpBass_sizelimits2) <- c("2004-2012 CRFS", "12-Inch MSL Simulation (Knife-Edge Selectivity)", "2013-2021 CRFS", "14-Inch MSL Simulation (Knife-Edge Selectivity)", "16-Inch MSL Simulation (Knife-Edge Selectivity)")
...
#VIOLIN
ggplot(KelpBass_sizelimits_gg2,aes(Scenario,SPR, group=Scenario)) +
  scale_color_manual(values=c("black", "red")) +
  scale_y_continuous(limits = c(0,1), expand = c(0, 0)) +
  annotation_custom(grob = green, xmin = -Inf, xmax = Inf, ymin = .5, ymax = 1)+
  annotation_custom(grob = green_yellow, xmin = -Inf, xmax = Inf, ymin = .3, ymax = .6)+
...
```{r}
KelpBass_sizelimits_14 <- data.frame(cbind(KB_LH_POST_LBSPR.LinfMK$SPR, LHKB_PRE$SPR_14_Inch_MS_KE))
colnames(KelpBass_sizelimits_14) <- c("2013-2021 CRFS", "14-Inch MSL Simulation (Knife-Edge Selectivity)")
KelpBass_sizelimits_gg14<-melt(KelpBass_sizelimits_14,variable.name="Data",value.name="SPR")
colnames(KelpBass_sizelimits_gg14)<-c("Scenario","SPR")

library(scales)

#VIOLIN
ggplot(KelpBass_sizelimits_gg14,aes(Scenario,SPR, group=Scenario)) +
  scale_color_manual(values=c("black", "red")) +
  scale_y_continuous(limits = c(0,1), expand = c(0, 0)) +
    annotation_custom(grob = green, xmin = -Inf, xmax = Inf, ymin = .5, ymax = 1)+
  annotation_custom(grob = green_yellow, xmin = -Inf, xmax = Inf, ymin = .3, ymax = .6)+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  ggtitle("Does the Simulated MSL Match the Data?", "Kelp Bass")+
...
#ggsave("KelpBass_Stochastic_14MSL.png", width = 30, height = 20, units = "cm", dpi = 500)
```
